 ...
 */
static double col_tile[KIC][32768];

// #((112 + 2*1 - 3)/1 + 1) * ((112 + 2*1 - 3)/1 + 1) = 12544

/* Output matrix: out_[C_out][H_out * W_out] */
static double out_[OUT_C][36864]; /* named out_ to avoid 'out' keyword */
...
{// 1. 바깥 루프는 타일 개수만큼 반복 (Stride 1)
    for (int oh = 0; oh < 192; ++oh) {
        for (int ow0_idx = 0; ow0_idx < (192 + 32768 - 1) / 32768; ++ow0_idx) {
#pragma scop
            for (int idx = 0; idx < 32768; ++idx) {
                if (ow0_idx * 32768 + idx < 192) {
                    for (int c = 0; c < IN_C; ++c) {
                        for (int kh = 0; kh < K_H; ++kh) {
                            for (int kw = 0; kw < K_W; ++kw) {
                                if (0 <= oh - 1 + kh && oh - 1 + kh < 192 &&
                                    0 <= ow0_idx * 32768 + idx - 1 + kw &&
                                    ow0_idx * 32768 + idx - 1 + kw < 192) {
                                    col_tile[c * 9 + kh * 3 + kw][idx] =
                                        x[c][oh - 1 + kh][ow0_idx * 32768 + idx - 1 + kw];
                                } else {
            ...
            for (int oc = 0; oc < OUT_C; ++oc) {
                for (int idx = 0; idx < 32768; ++idx) {
                    if (ow0_idx * 32768 + idx < 192) {
                        for (int k = 0; k < KIC; ++k) {
                            out_[oc][oh * 192 + ow0_idx * 32768 + idx] +=
