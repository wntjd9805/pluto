...
#define H_OUT  ((IN_H + 2*PAD - K_H)/STRIDE + 1)
#define W_OUT  ((IN_W + 2*PAD - K_W)/STRIDE + 1)
 ...
 */
static double col_tile[KIC][1024];

// #((112 + 2*1 - 3)/1 + 1) * ((112 + 2*1 - 3)/1 + 1) = 12544

/* Output matrix: out_[C_out][H_out * W_out] */
static double out_[OUT_C][1024]; /* named out_ to avoid 'out' keyword */
...
{// 1. 바깥 루프는 타일 개수만큼 반복 (Stride 1)
    for (int hw0_idx = 0; hw0_idx < (1024 + 128 - 1) / 128; ++hw0_idx) {
        #pragma scop
        for (int idx = 0; idx < 128; ++idx) {
            for (int c = 0; c < IN_C; ++c) {
                for (int kh = 0; kh < K_H; ++kh) {
                    for (int kw = 0; kw < K_W; ++kw) {
                        if (hw0_idx * 128 + idx < 1024) {
                            if ((hw0_idx * 4 + idx / 32- 1 + kh) < 32 &&
                                (idx % 32 - 1 + kw) < 32) {
                                col_tile[c * 9 + kh * 3 + kw][idx] =
                                    x[c][(hw0_idx * 4 + idx/ 32) - 1 + kh]
                                         [(idx % 32) - 1 + kw];
                            } else {
...
#pragma scop
        for (int oc = 0; oc < OUT_C; ++oc) {
            for (int idx = 0; idx < 128; ++idx) {
                for (int k = 0; k < KIC; ++k) {
                    if (hw0_idx * 128 + idx < 1024) {
                        out_[oc][hw0_idx * 128 + idx] += Wpk[oc][k] * col_tile[k][idx];
                    }
                }
            }
        }
