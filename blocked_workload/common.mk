...
NTHREADS=48
ifeq ("$(wildcard $(POLYBENCH_UTIL_DIR)/polybench.c)","")
POLYBENCH_UTIL_DIR := $(HYPERF_HOME)/polybench/OpenMP/utilities
endif
ifeq ("$(wildcard $(POLYBENCH_UTIL_DIR)/polybench.c)","")
$(error polybench.c not found. checked $(BASEDIR)utilities and $(HYPERF_HOME)/polybench/OpenMP/utilities)
endif
POLYBENCHINCDIR := $(POLYBENCH_UTIL_DIR)
POLYBENCHSRC := $(POLYBENCH_UTIL_DIR)/polybench.c
PLC ?= $(BASEDIR)polycc
ifeq ("$(wildcard $(PLC))","")
PLC := $(shell command -v polycc 2>/dev/null)
endif
ifeq ("$(strip $(PLC))","")
$(error polycc not found. expected $(BASEDIR)polycc or polycc in PATH)
endif
...
ifeq ($(CC), icc)
	...
	OPT_FLAGS     := -I/usr/lib/ -I$(POLYBENCHINCDIR) -I$(BASEDIR) -I. -DPOLYBENCH_TIME -O3 -march=$(TARGET_MARCH)
	PAR_FLAGS     := --parallel 
	...
	# for gcc
	OPT_FLAGS     := -I$(POLYBENCHINCDIR) -I$(BASEDIR) -I. -DPOLYBENCH_TIME -O3 -march=$(TARGET_MARCH)
...
perf: orig tiled par orig_par
	rm -f .test
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./orig
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./orig_par
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./tiled
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./par 

# Compare performance with and without diamond tiling.
pipeperf: par pipepar
	rm -f .test
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close	 ./par
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./pipepar 

test: orig tiled par
	touch .test
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close  ./orig 2> out_orig
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close  ./tiled 2> out_tiled
	diff -q out_orig out_tiled
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./par 2> out_par4
	rm -f .test
...
lbtest: par pipepar
	touch .test
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./par 2> out_par4
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./pipepar 2> out_pipepar4
	rm -f .test
...
opt-test: orig opt
	touch .test
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./orig > out_orig
	OMP_NUM_THREADS=$(NTHREADS) OMP_PROC_BIND=close ./opt > out_opt
	rm -f .test
