          ...
          for (int j = 0; j < _PB_NI; ++j) {
            for (int kk = 0; kk < BLOCK_SIZE; ++kk) {
              if ((bi * 128 + ii) < _PB_NI &&
                  (bk * 128 + kk) < _PB_NI) {
                B[bi * 128 + ii][j] +=
                    A[bk * 128 + kk][bi * 128 + ii] *
                    B_read[(bk * 128 + kk) * 1024 + j];
              }
            }
          }
        }
      ...
      for (int ii = 0; ii < BLOCK_SIZE; ++ii) {
        for (int j = 0; j < _PB_NI; ++j) {
          for (int t = ii + 1; t < BLOCK_SIZE; ++t) {
            if ((bi * 128 + ii) < _PB_NI && (bi * 128 + t) < _PB_NI) {
              B[bi * 128 + ii][j] +=
                  A[bi * 128 + t][bi * 128 + ii] *
                  B_read[(bi * 128 + t) * 1024 +j]; // 읽기는 스냅샷, 쓰기는 B
            }
          }
        }
      }
